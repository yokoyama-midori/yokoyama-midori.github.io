...
## 11/26
switch文に対応した。リファレンス実装はbreak文がなくてもフォールスルーが起こらないような実装になっている気がするが、実際はスルーが起きると思う。  
関数の中身が空であるときにバグがあったので直した。初期化をしていなかったのが原因。

## 11/27
`<<`,`>>`,`<<=`,`>>=`に対応した。右シフトは符号付き整数にしか対応していない。  
三項演算子に対応した。  
定数式に対応した。

## 12/1
初期化リストによる配列の初期化に対応した。少し難しかったのでリファレンスをよく見ながら実装した。リストの要素数が足りない場合は0で埋めるらしい。`{0}`を使うことで全要素をヌルで初期化できる理由が分かった。
...
int x[] = {1, 2, 3};
```
のような、不完全な型に対する初期化に対応した。


## 1/17
initializerを使った構造体の初期化に対応した。

結構始めたときから時間が経って、当初のやる気も無くなってきたので気分転換でこれまで使ってきたVSCodeからWindows Terminal+NeoVimに移行してみている。特にLazyVimというのを使っていて、これはかなり高機能かつ最初から使いやすいようにいろいろ設定されてあるので、なるべくキー操作などは与えられたものを使って見るようにしている。シンタックスハイライトについては[このブログ](https://tonsky.me/blog/syntax-highlighting/)を読んで割と共感できるところが多かったので[alabaster.nvim](https://github.com/p00f/alabaster.nvim)を使ってみている。全部目立たせると結局なにも目立ってないのと同じだから、必要なところだけ目立たせようみたいな内容。コメントを目立たせるのが特徴。Vim操作については、まだまだ快適ではないが色々ヒントを出してくれるのでそこまで大変ではない印象。ただ、こういうマークダウンファイルを日本語で書くのには向かないと思うのでこちらはVSCodeを引き続き使っている。
...
```
というコードが動かなくて悩んでいた。
`push offset st`とコンパイルされた部分が
`Error: invalid use of register`とエラーが出ていた。昔のレジスタの名前（？）と衝突していたらしい。

## 1/23,25
initializerによるグローバル変数の初期化に対応した。  
ところで、僕は`&&`,`||`の代わりに`and`,`or`を使っている。競技プログラミングの際に、`and`,`or`の方が速く正確に打てるからという理由で使い始めたのがきっかけだ。NeoVimを使い始めてから次のようなコードが正しくシンタックスハイライトされていないことに気づいた。

```c
do {
  // ...
} while (!peek_end() and consume(","));
```

`and`を`&&`に変えたり、適当に括弧を追加すると正しくハイライトされた。また、C++で同様のコードを書いてみても、正しく表示された。
Cの`and`,`or`はiso646.hというヘッダファイルで`#define`を使って定義されているが、Tree-sitterというパーサ生成ツールがマクロを展開しないことが原因でこのようなことが起きるようだ。[issue](https://github.com/nvim-treesitter/nvim-treesitter/issues/5612)に似たような話があり、LSPを使うべきということらしい。clangdは既に入れてあるので、Tree-sitterのシンタックスハイライトをオフにしたところ正しく表示された。